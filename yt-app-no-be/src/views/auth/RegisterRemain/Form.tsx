...
import useUserTempExist from '@hooks/api/useUserTempExist';
...
const FormRegister = () => {
  ...
  const errorToast = (e?: string) =>
    toast(e ? e : t('pages.register.toast.error'), { type: 'error' });

  const secretCryptoAes = process.env.NEXT_PUBLIC_CRYPTO_SECRET;

  const userId = params?.userId as string;

  const hashedUserId = CryptoJS.AES.encrypt(
    userId,
    secretCryptoAes as string
  ).toString();

  const { user, error } = useUserTempExist({
    id: userId,
    hashId: hashedUserId,
  });
  ...
  const onSubmit: SubmitHandler<RegisterSchemaType> = async (data) => {
    loading.onOpen();
    axios
      .put(`/auth/register-remaining/${userId}`, data)
      .then((res) => {
      ...
      <form className='w-f mt-6' onSubmit={handleSubmit(onSubmit)}>
        <div className='mb-6'>
          ...
          <input
            type='email'
            id='email'
            className='block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-yt-gray-100  p-2.5 text-sm text-gray-900 focus:border-yt-blue-500 focus:ring-yt-blue-500 dark:border-yt-gray-600 dark:bg-yt-gray-100 dark:text-black dark:placeholder-gray-400 dark:focus:border-yt-blue-500 dark:focus:ring-yt-blue-500'
